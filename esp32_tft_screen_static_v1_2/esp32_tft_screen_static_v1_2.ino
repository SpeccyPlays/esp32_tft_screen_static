...
// 2022/04/06
// Turning code in to a class
#include <SPI.h>
#include <TFT_eSPI.h>
unsigned char Testcard_F[] PROGMEM = {
...
class screenStatic {
  public:
    void start(byte tempHeight, byte tempWidth, byte tempStaticHeight, byte tempPixelSize, unsigned char *tempBackground){
  ...
  void displayStaticBar(){
    for (byte i = 0; i < screenWidth; i += staticPixelSize){
      for (byte y = i; y < (i + staticStripHeight); y +=staticPixelSize){
        for (byte j = 0; j < screenHeight; j += staticPixelSize){
          byte pixel = random(0, 10);
          if (pixel <= 5){
            myScreen.fillRect(j, y, staticPixelSize, staticPixelSize, TFT_WHITE);
          }
          else {
      ...
      myScreen.fillRect(0, i, screenHeight, 2, TFT_BLACK);
      myScreen.drawXBitmap(0 ,0, background, screenHeight, screenWidth, TFT_WHITE);
      delay(2);
      }
    }
...
void setup(){
  staticOne.start(SCREENHEIGHT, SCREENWIDTH, 5, 2, Testcard_F);
  delay(5000);
}
void loop() {
